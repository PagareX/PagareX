...
{
    LogPrintf("PagareXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pagarex-miner");
	...
	CBlock *pblock = pblocktemplate.get();
        ...
        LogPrintf("Running PagareXMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("PagareXMiner terminated\n");
        throw;
    }
}
